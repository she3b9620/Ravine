...
export default async function DiscoverPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<SearchParams> }) {
  ...
  return (
    <section className="section discover-page">
      ...
      <DiscoverFilters locale={locale} query={query ?? ""} category={filters.category ?? ""} type={filters.type ?? ""} duration={filters.duration ?? ""} format={filters.format ?? ""} quality={filters.quality ?? ""} sort={filters.sort ?? "newest"} categories={normalizedCategories} />
      {error ? (
        <div className="empty-state"><strong>{isArabic ? "تعذر تحميل الأعمال." : "We could not load the work."}</strong><span>{errorDetail}</span></div>
      ) : videos.length === 0 ? (
        <div className="empty-state"><strong>{isArabic ? "لا توجد أعمال مطابقة حاليًا." : "No matching work yet."}</strong><span>{isArabic ? "جرّب بحثًا أو فلترًا مختلفًا." : "Try another search or filter."}</span></div>
      ) : (
        <div className="video-grid" id="discover-results">
          {videos.map((video) => (
            <Link href={`/${locale}/watch/${video.id}`} className="video-card" key={video.id}>
              <div className="video-thumb"><img src={video.thumbnail_url || "/RAVINE.png"} alt="" loading="lazy" /><span className="duration">{durationLabel(video.duration)}</span></div>
              <div className="video-meta"><div className="video-kicker">{video.content_type || "WORK"}{video.quality ? ` · ${video.quality}` : ""}</div><h2>{video.title || (isArabic ? "بدون عنوان" : "Untitled")}</h2><p>{video.description || (isArabic ? "عمل إبداعي من مجتمع رَافِين." : "A creative work from the RAVINE community.")}</p><div className="video-stats"><span>{formatRavineNumber(Number(video.views || 0), locale)} {isArabic ? "مشاهدة" : "views"}</span><span>{formatRavineNumber(Number(video.likes || 0), locale)} {isArabic ? "إعجاب" : "likes"}</span></div></div>
            </Link>
          ))}
        </div>
      )}

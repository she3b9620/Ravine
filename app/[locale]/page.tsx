...
const copy = {
  ar: {
    ...
    aboutBody:
      "RAVINE مساحة حول قيمة العمل: تكتشف العمل، تفهم سياقه، تصل إلى صانعيه واعتماداته، ثم تنتقل إلى المجتمع وLive وPodcast.",
    quality: "الجودة قبل الضجيج.",
    qualityBody:
      "التقدير، الاكتشاف، والسمعة أنظمة مختلفة. النجاح هنا لا يُختزل في عدد المشاهدات.",
  },
  en: {
    eyebrow: "RAVINE / 01",
    ...
    selectionsBody:
      "One editorial horizon that changes by time period instead of becoming four duplicated feeds.",
    daily: "Daily",
    weekly: "Weekly",
    monthly: "Monthly",
    yearly: "Yearly",
    ...
    aboutBody:
      "RAVINE is a space around the value of work: discover it, understand its context, find the people and credits behind it, then continue into community, Live, and Podcast experiences.",
    quality: "Quality before noise.",
    qualityBody:
      "Rating, discovery, and reputation remain distinct. Success here is never reduced to a view count.",
  },
} as const;
  ...
  return (
    <Link href={`/${locale}/watch/${work.id}`} className="video-card">
      ...
      <div className="video-meta">
        ...
        <h2>
          {work.title || (locale === "ar" ? "عمل بدون عنوان" : "Untitled work")}
        </h2>
        <p>{getCreatorLabel(work.creators, locale)}</p>
        <div className="video-stats">
          <span>
            {Number(work.views || 0).toLocaleString()} {views}
          </span>
          <span>
            {Number(work.likes || 0).toLocaleString()} {likes}
          </span>
        </div>
      </div>
    </Link>
...
export default async function HomePage({
  ...
  try {
    ...
    const [worksResult, creatorsResult] = await Promise.all([
      supabase
        .from("videos")
        .select(
          "id,title,description,thumbnail_url,duration,views,likes,content_type,quality,creators(name,username)",
        )
        .eq("published", true)
  ...
  const liveWorks = works
    .filter((work) => work.content_type === "live")
    .slice(0, 3);
  const podcastWorks = works
    .filter((work) => work.content_type === "podcast")
    .slice(0, 3);
  const discoveryWork = works
    .filter(
      (work) =>
        work.content_type !== "live" && work.content_type !== "podcast",
    )
    .slice(0, 4);
    ...
    <>
      <section className="hero">
        <div className="hero-inner">
          ...
          <div className="hero-actions">
            <Link className="button primary" href={`/${locale}/discover`}>
              {t.discover}
            </Link>
      ...
      <section className="section selection-section">
        <div className="section-head">
          ...
          <div
            ...
            aria-label={locale === "ar" ? "مدة المختارات" : "Selection period"}
          >
      ...
      <section className="section">
        <div className="section-head">
          ...
          <Link className="button secondary" href={`/${locale}/discover`}>
            {t.discover}
          </Link>
        </div>
        <div className="pathway-grid">
          <Link
            href={`/${locale}/discover?type=video`}
          ...
          >
            ...
            <p>
              {locale === "ar"
                ? "حركة الأعمال والانتباه إليها الآن."
                : "Current movement and attention around work."}
            </p>
          </Link>
          <Link
            href={`/${locale}/discover?type=short`}
            className="pathway-card"
          >
            <span>02</span>
            <strong>{t.rising}</strong>
            <p>
              {locale === "ar"
                ? "مساحات للأعمال والمبدعين الصاعدين."
                : "A path for emerging work and creators."}
            </p>
          </Link>
          <Link
            href={`/${locale}/discover?type=video`}
          ...
          >
            ...
            <p>
              {locale === "ar"
                ? "واجهة للاكتشاف حتى عندما يكون التعرض منخفضًا."
                : "A discovery surface for strong work with low exposure."}
            </p>
          </Link>
          <Link href={`/${locale}/discover`} className="pathway-card">
            <span>04</span>
            <strong>{t.featuredPath}</strong>
            <p>
              {locale === "ar"
                ? "ما تضعه RAVINE في واجهة التجربة."
                : "Work the platform chooses to spotlight."}
            </p>
          </Link>
        </div>
      </section>
          ...
          <div className="creator-strip">
            {creators.map((creator) => (
              <Link
                ...
                className="creator-feature"
              >
                <div className="creator-feature-avatar">
                  <img
      ...
      <section className="section media-section">
        <div className="media-column">
          <div className="section-head compact">
            <div>
              <div className="eyebrow">RAVINE / LIVE</div>
              <h2>{t.liveTitle}</h2>
            </div>
            <Link className="button secondary" href={`/${locale}/live`}>
              {t.discover}
            </Link>
          </div>
          {liveWorks.length ? (
        ...
        <div className="media-column">
          <div className="section-head compact">
            <div>
              <div className="eyebrow">RAVINE / PODCAST</div>
              <h2>{t.podcastTitle}</h2>
            </div>
            <Link className="button secondary" href={`/${locale}/podcasts`}>
              {t.discover}
            </Link>
          </div>
          {podcastWorks.length ? (
          ...
          <div>
            ...
            <div className="about-principles">
              <div>
                ...
                <span>{t.qualityBody}</span>
              </div>
              <div>
                <strong>
                  {locale === "ar" ? "المبدعون لا القنوات." : "Creators, not channels."}
                </strong>
                <span>
                  {locale === "ar"
                    ? "الهوية المهنية والعمل والاعتمادات أهم من واجهة قناة عامة."
                    : "Professional identity, work, and credits matter more than a generic channel shell."}
                </span>
              </div>
              <div>
                <strong>
                  {locale === "ar"
                    ? "الرسم البياني الإبداعي."
                    : "The Creative Graph."}
                </strong>
                <span>
                  {locale === "ar"
                    ? "من العمل إلى صانعيه واعتماداته ومجتمعه ثم Live وPodcast."
                    : "Move from a work to its creators, credits, community, Live, and Podcast."}
                </span>
              </div>
            </div>
          </div>

...
const copy = {
  ar: {
    ...
    viewerBody: "موجز غني من الأعمال الحديثة، ما يتابعُه المستخدم، ومسارات اكتشاف متجاورة — تجربة ممتدة وليست لوحة فارغة.",
    forYou: "لك",
    following: "تتابعهم",
    followingEmpty: "تابع بعض المبدعين وسيظهر محتواهم هنا.",
    browseCreators: "اكتشف المبدعين",
    latest: "الأحدث",
    trending: "الأكثر حركة",
  ...
  en: {
    ...
    viewerBody: "A rich stream of recent work, what you follow, and adjacent discovery paths — a living home rather than an empty dashboard.",
    forYou: "For You",
    following: "Following",
...
function Rail({ title, eyebrow, action, href, works, locale, compact = false }: { title: string; eyebrow: string; action?: string; href?: string; works: HomeWork[]; locale: Locale; compact?: boolean }) {
  return (
    <section className={`section home-rail-section${compact ? " compact" : ""}`}>
      <div className="home-rail-head">
        <div><div className="home-feed-label">{eyebrow}</div><h2>{title}</h2></div>
        {href && action ? <Link className="home-rail-link" href={href}>{action} <span aria-hidden="true">→</span></Link> : null}
      </div>
      {works.length ? <div className="home-rail">{works.map((work) => <WorkCard key={work.id} work={work} locale={locale} compact={compact} />)}</div> : null}
    </section>
  );
}
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  try {
    ...
    works = (worksResult.data ?? []) as HomeWork[];
    ...
    if (user) {
      ...
      if (ids.length) {
        const followingResult = await supabase.from("videos").select("id,title,description,thumbnail_url,video_url,duration,views,likes,content_type,quality,creator_id,creators(name,username)").eq("published", true).in("creator_id", ids).order("created_at", { ascending: false }).limit(18);
        if (!followingResult.error) followingWorks = (followingResult.data ?? []) as HomeWork[];
      }
    }
  } catch {
    works = [];
  ...
  const liveWorks = works.filter((work) => work.content_type === "live").slice(0, 4);
  const podcastWorks = works.filter((work) => work.content_type === "podcast").slice(0, 4);
  const shortsWorks = works.filter((work) => work.content_type === "short").slice(0, 10);
  const latestWorks = publicWorks.slice(0, 12);
  const trendingWorks = [...publicWorks].sort((a, b) => Number(b.views || 0) - Number(a.views || 0)).slice(0, 12);
  const hiddenGemWorks = [...publicWorks].sort((a, b) => Number(a.views || 0) - Number(b.views || 0)).slice(0, 12);
  ...
  if (user) {
    return (
      <div className="home-viewer">
        ...
        <section className="section home-primary-feed home-feed-section">
          <div className="home-feed-head">
            <div><div className="home-feed-label">{locale === "ar" ? "رَافِين / الموجز" : "RAVINE / FEED"}</div><h1>{t.forYou}</h1><p className="home-feed-intro">{locale === "ar" ? "أعمال مختارة من أحدث الإصدارات، مع المعاينة الصامتة عند الوقوف على الصورة." : "Recent work with silent hover previews when you pause over a thumbnail."}</p></div>
            <Link className="button secondary" href={`/${locale}/discover`}>{t.more}</Link>
          </div>
          {latestWorks.length ? <div className="home-feed-grid home-dense-grid">{latestWorks.map((work) => <WorkCard key={work.id} work={work} locale={locale} />)}</div> : <div className="empty-state"><strong>{t.empty}</strong></div>}
        </section>
        ...
        {shortsWorks.length ? <Rail title={t.shorts} eyebrow="RAVINE / CUTS" action={t.more} href={`/${locale}/cuts`} works={shortsWorks} locale={locale} compact /> : null}
        <Rail title={t.hiddenGems} eyebrow={locale === "ar" ? "رَافِين / مخفية" : "RAVINE / HIDDEN"} action={t.more} href={`/${locale}/discover?sort=hidden`} works={hiddenGemWorks} locale={locale} />

        <section className="section home-creator-discovery">
          <div className="section-head"><div><div className="eyebrow">{locale === "ar" ? "رَافِين / الأشخاص" : "RAVINE / PEOPLE"}</div><h2>{t.creatorsTitle}</h2><p className="section-note">{t.creatorsBody}</p></div><Link className="button secondary" href={`/${locale}/creators`}>{t.browseCreators}</Link></div>
          {creators.length ? <div className="creator-strip">{creators.map((creator) => <CreatorCard key={creator.id} creator={creator} locale={locale} />)}</div> : <div className="empty-state"><strong>{t.emptyCreators}</strong></div>}
        </section>
        ...
        <section className="section selection-section">
          ...
          <div className="video-grid">{publicWorks.slice(0, 6).map((work) => <WorkCard key={work.id} work={work} locale={locale} />)}</div>
        </section>

        <section className="section home-media-split">
          <div className="media-column"><div className="section-head compact"><div><div className="eyebrow">{locale === "ar" ? "رَافِين / مباشر" : "RAVINE / LIVE"}</div><h2>{t.liveTitle}</h2></div><Link className="button secondary" href={`/${locale}/live`}>{t.more}</Link></div>{liveWorks.length ? <div className="mini-grid">{liveWorks.map((work) => <WorkCard key={work.id} work={work} locale={locale} compact />)}</div> : <div className="empty-state"><strong>{t.emptyLive}</strong></div>}</div>
          <div className="media-column"><div className="section-head compact"><div><div className="eyebrow">{locale === "ar" ? "رَافِين / صوت" : "RAVINE / AUDIO"}</div><h2>{t.podcastTitle}</h2></div><Link className="button secondary" href={`/${locale}/podcasts`}>{t.more}</Link></div>{podcastWorks.length ? <div className="mini-grid">{podcastWorks.map((work) => <WorkCard key={work.id} work={work} locale={locale} compact />)}</div> : <div className="empty-state"><strong>{t.emptyPodcast}</strong></div>}</div>
        </section>

        <section className="section home-community-teaser"><div className="section-head"><div><div className="eyebrow">{t.communityEyebrow}</div><h2>{t.communityTitle}</h2><p className="section-note">{t.communityBody}</p></div><Link className="button secondary" href={`/${locale}/community`}>{locale === "ar" ? "دخول المجتمعات" : "Enter communities"}</Link></div><div className="home-community-path"><span>{t.communityGeneral}</span><i>→</i><span>{t.communityTopics}</span><i>→</i><span>{t.communityCreators}</span><i>→</i><span>{t.communityGroups}</span></div><p className="home-community-loop">{t.communityLoop}</p></section>
      </div>
    );
  }
  ...
  return (
    <div className="home-guest">
      ...
      <section className="section"><div className="home-guest-preview"><div><div className="eyebrow">{locale === "ar" ? "رَافِين / معاينة" : "RAVINE / PREVIEW"}</div><h2>{t.guestPreview}</h2><p className="home-guest-intro">{t.guestPreviewBody}</p><div className="video-grid">{publicWorks.slice(0, 6).map((work) => <WorkCard key={work.id} work={work} locale={locale} />)}</div></div><div className="home-guest-note"><strong>{locale === "ar" ? "شاهد، تعرّف، ثم انضم." : "See it. Understand it. Then join."}</strong><p>{locale === "ar" ? "التصفح العام متاح للضيوف، بينما الإعجاب والحفظ والمتابعة والتقييم والمجتمعات والموجز الشخصي تحتاج إلى حساب." : "Public browsing stays open to guests; liking, saving, following, rating, community actions and a personal feed require an account."}</p></div></div></section>
      ...
      <section className="section home-community-teaser"><div className="section-head"><div><div className="eyebrow">{t.communityEyebrow}</div><h2>{t.communityTitle}</h2><p className="section-note">{t.communityBody}</p></div><Link className="button secondary" href={`/${locale}/community`}>{locale === "ar" ? "استكشف المجتمعات" : "Explore communities"}</Link></div><div className="home-community-path"><span>{t.communityGeneral}</span><i>→</i><span>{t.communityTopics}</span><i>→</i><span>{t.communityCreators}</span><i>→</i><span>{t.communityGroups}</span></div><p className="home-community-loop">{t.communityLoop}</p></section>
      <section className="section selection-section"><div className="section-head"><div><div className="eyebrow">{locale === "ar" ? "رَافِين / المختارات" : "RAVINE / SELECT"}</div><h2>{t.selections}</h2><p className="section-note selection-note">{t.selectionsBody}</p></div><div className="selection-tabs"><span className="selection-tab active">{t.daily}</span><span className="selection-tab">{t.weekly}</span><span className="selection-tab">{t.monthly}</span><span className="selection-tab">{t.yearly}</span></div></div>{publicWorks.length ? <div className="video-grid">{publicWorks.slice(0, 6).map((work) => <WorkCard key={work.id} work={work} locale={locale} />)}</div> : <div className="empty-state"><strong>{t.empty}</strong></div>}</section>
      <section className="section media-section"><div className="media-column"><div className="section-head compact"><div><div className="eyebrow">{locale === "ar" ? "رَافِين / جلسات مباشرة" : "RAVINE / LIVE"}</div><h2>{t.liveTitle}</h2></div><Link className="button secondary" href={`/${locale}/live`}>{t.discover}</Link></div>{liveWorks.length ? <div className="mini-grid">{liveWorks.map((work) => <WorkCard key={work.id} work={work} locale={locale} compact />)}</div> : <div className="empty-state"><strong>{t.emptyLive}</strong></div>}</div><div className="media-column"><div className="section-head compact"><div><div className="eyebrow">{locale === "ar" ? "رَافِين / برامج صوتية" : "RAVINE / PODCAST"}</div><h2>{t.podcastTitle}</h2></div><Link className="button secondary" href={`/${locale}/podcasts`}>{t.discover}</Link></div>{podcastWorks.length ? <div className="mini-grid">{podcastWorks.map((work) => <WorkCard key={work.id} work={work} locale={locale} compact />)}</div> : <div className="empty-state"><strong>{t.emptyPodcast}</strong></div>}</div></section>
    </div>
  );
}

...
import { ArrowUpRight, BadgeCheck, Edit3, ExternalLink, History, LayoutDashboard, Library, Play, Settings2, Sparkles, Users } from "lucide-react";
    ...
    <section className={`section account-page ${styles.page}`} dir={ar ? "rtl" : "ltr"}>
      <header className={`${styles.hero} ${isCreator ? styles.creatorHero : ""}`}>
        <div className={styles.heroCopyBlock}>
          ...
          <div className={styles.heroActions}><Link className={styles.primaryButton} href={`/${locale}/account/edit`}><Edit3 size={15} />{ar ? "تعديل الملف" : "Edit profile"}</Link>{publicProfileHref ? <Link className={styles.secondaryButton} href={publicProfileHref}><ExternalLink size={15} />{ar ? "مشاهدة صفحتي" : "View my page"}</Link> : null}{isCreator ? <Link className={styles.secondaryButton} href={`/${locale}/studio`}><Sparkles size={15} />{ar ? "استوديو المبدع" : "Creator Studio"}</Link> : <Link className={styles.secondaryButton} href={`/${locale}/discover`}><ArrowUpRight size={15} />{ar ? "ابدأ اكتشافًا" : "Start discovering"}</Link>}</div>
        </div>
      ...
      {publicProfileHref ? <section className={styles.publicProfileCallout}><div><div className={styles.eyebrow}>{ar ? "رَافِين / صفحتك العامة" : "RAVINE / YOUR PUBLIC PAGE"}</div><h2>{ar ? "شاهد كيف يراك الناس خارج التعديل." : "See how people see you outside the editor."}</h2><p>{ar ? "هذه هي صفحة البروفايل العادية، منفصلة تمامًا عن إعدادات الحساب والتعديل." : "This is your normal public profile page, completely separate from account editing."}</p></div><Link className={styles.primaryButton} href={publicProfileHref}><ExternalLink size={15}/>{ar?"فتح صفحتي":"Open my page"}</Link></section> : null}

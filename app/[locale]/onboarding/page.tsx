...
export default function OnboardingPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { error: profileError } = await supabase.from("profiles").upsert({ id: userData.user.id, display_name: cleanDisplay, username: cleanUsername, language, country: country.trim() || null, updated_at: new Date().toISOString() }, { onConflict: "id" });
  ...
  return <section className="auth-page" dir={ar ? "rtl" : "ltr"}>
    <div className="auth-card">
      ...
      <form onSubmit={submit} className="auth-form">
        ...
        <label>{ar ? "اسم المستخدم" : "Username"}<input value={username} onChange={(e) => setUsername(e.target.value)} required minLength={3} maxLength={30} pattern="[A-Za-z0-9_]{3,30}" autoCapitalize="none" autoComplete="username" placeholder="yourname" /></label>
        <label>{ar ? "اللغة" : "Language"}<select value={language} onChange={(e) => setLanguage(e.target.value as "ar" | "en")} required><option value="ar">{ar ? "العربية" : "Arabic"}</option><option value="en">{ar ? "الإنجليزية" : "English"}</option></select></label>
        <label>{ar ? "البلد (اختياري)" : "Country (optional)"}<input value={country} onChange={(e) => setCountry(e.target.value)} maxLength={80} autoComplete="country-name" /></label>
        <button className="button primary auth-submit" type="submit" disabled={saving}>{saving ? "…" : ar ? "حفظ والمتابعة للأمان" : "Save and continue to security"}</button>
      </form>

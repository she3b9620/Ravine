...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(async ({ data, error: userError }) => {
      ...
      const { data: profile } = await supabase.from("profiles").select("display_name,username,country").eq("id", data.user.id).maybeSingle();
      if (!mounted) return;
      ...
      if (mounted) setError(ar ? "تعذر تحميل حسابك." : "Could not load your account.");
    });
  ...
  return (
    <section className="auth-page">
      <div className="auth-card">
        ...
        <p className="auth-intro">{ar ? "اسم العرض واسم المستخدم أساسيان. البلد اختياري ويمكن تغييره لاحقًا." : "A display name and username are essential. Country is optional and can change later."}</p>
        <form onSubmit={submit} className="auth-form">
          ...
          <button className="button primary auth-submit" type="submit" disabled={saving}>{saving ? "…" : ar ? "حفظ والدخول" : "Save and enter"}</button>
        </form>

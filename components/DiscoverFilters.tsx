...
const formats = [
  ...
  { value: "other", ar: "نسبة أخرى", en: "Other ratio" },
];
...
export default function DiscoverFilters({
  ...
  return (
    <form ref={rootRef} className="discover-filters" action={`/${locale}/discover`}>
        ...
        {open === "category" ? (
          <div className="discover-filter-menu" role="listbox" aria-label={isArabic ? "اختيار التصنيف" : "Choose category"}>
            <button type="button" className={!selectedCategory ? "is-selected" : ""} onClick={() => { setSelectedCategory(""); setOpen(null); }}>
              ...
              {!selectedCategory && <Check size={15} />}
            </button>
            {categories.map((item) => {
              const active = String(item.id) === selectedCategory;
              return (
                <button type="button" className={active ? "is-selected" : ""} key={item.id} onClick={() => { setSelectedCategory(String(item.id)); setOpen(null); }}>
                  <span>{item.name}</span>
        ...
        {open === "type" ? (
          <div className="discover-filter-menu" role="listbox" aria-label={isArabic ? "اختيار النوع" : "Choose type"}>
            ...
            {types.map((item) => {
              ...
              return (
                <button type="button" className={active ? "is-selected" : ""} key={item.value} onClick={() => { setSelectedType(item.value); setOpen(null); }}>
      ...
      <button
        className={`discover-more-trigger${advancedOpen ? " is-open" : ""}${advancedCount ? " has-active" : ""}`}
        type="button"
        aria-expanded={advancedOpen}
        onClick={() => { setOpen(null); setAdvancedOpen(!advancedOpen); }}
      >
        <SlidersHorizontal size={17} />
        <span>{isArabic ? "فلاتر إضافية" : "More filters"}</span>
      ...
      <div className={`discover-filter-sheet${advancedOpen ? " is-open" : ""}`} aria-hidden={!advancedOpen}>
        ...
        <aside className="discover-filter-panel" role="dialog" aria-modal="true" aria-label={isArabic ? "فلاتر الاكتشاف" : "Discover filters"}>
          ...
          <div className="discover-filter-panel-foot">
            <button type="button" className="discover-clear" onClick={clearFilters}><RotateCcw size={15} />{isArabic ? "مسح الفلاتر" : "Reset filters"}</button>
            <button type="button" className="button primary discover-apply" onClick={() => setAdvancedOpen(false)}>{isArabic ? "تم" : "Done"}</button>
          </div>
        </aside>
      </div>
...
function FilterSelect({
  ...
  return (
    <div className="discover-panel-field" ref={rootRef}>
      <small>{label}</small>
      <button type="button" className={`discover-panel-select${open ? " is-open" : ""}${value ? " has-value" : ""}`} onClick={() => setOpen(!open)} aria-expanded={open}>
        <span>{current ? current[locale] : (isArabic ? "كل الخيارات" : "Any")}</span>
        <ChevronDown size={16} />
      </button>
      {open ? (
        <div className="discover-panel-menu" role="listbox">
          <button type="button" className={!value ? "is-selected" : ""} onClick={() => { onChange(""); setOpen(false); }}>
            <span>{isArabic ? "كل الخيارات" : "Any"}</span>
            {!value && <Check size={15} />}
          </button>
          {options.map((option) => (
            <button type="button" key={option.value} className={value === option.value ? "is-selected" : ""} onClick={() => { onChange(option.value); setOpen(false); }}>

...
export default async function RavineShell({ locale, children }: { locale: Locale; children: ReactNode }) {
  ...
  return (
    <div className={`ravine-shell${isAuthenticated ? "" : " guest-shell"}`} lang={locale} dir={isArabic ? "rtl" : "ltr"}>
      ...
      <header className="ravine-header"><div className="ravine-header-inner"><div className="ravine-header-start"><SidebarToggle locale={locale} /><Link href={`/${locale}`} className="ravine-brand ravine-brand-lockup" aria-label="RAVINE"><RavineLockup /></Link></div><PrimaryNav locale={locale} authenticated={isAuthenticated} /><div className="ravine-header-actions"><MobileNav locale={locale} authenticated={isAuthenticated} /><SearchLauncher locale={locale} categories={categories} /><div className="ravine-header-social" aria-label={isArabic ? "التنبيهات والرسائل والراديو" : "Notifications, messages, and radio"}>{isAuthenticated ? <><NotificationBell locale={locale} /><ChatLauncher locale={locale} /><RadioPlayerLauncher locale={locale} /></> : null}<RadioLauncher locale={locale} />{isAuthenticated ? <AccountMenu locale={locale} displayName={displayName} username={profile?.username || null} avatarUrl={profile?.avatar_url || null} isCreator={Boolean(profile?.is_creator)} /> : null}{!isAuthenticated ? <AuthTrigger locale={locale} label={isArabic ? "دخول" : "Sign in"} mode="signin" /> : null}<div className="ravine-header-preferences"><ThemeToggle locale={locale} /><Suspense fallback={<span className="ravine-language" aria-hidden="true">{locale === "ar" ? "العربية" : "English"}</span>}><LanguageSwitcher locale={locale} /></Suspense></div></div></div></div></header>

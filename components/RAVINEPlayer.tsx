...
export default function RAVINEPlayer({ src, poster, title, contentType, duration, locale, chapters = [], assets = [] }: Props) {
  ...
  return (
    <section className={styles.player} dir={ar ? "rtl" : "ltr"} aria-label={ar ? "مشغل RAVINE" : "RAVINE player"}>
      <div className={styles.stage}>
        ...
        {activeAuxiliary ? (
          <div className={styles.auxiliaryLabel}>
            ...
            <button type="button" onClick={() => setIntroChoice("main")}>{ar ? "مشاهدة العمل" : "Watch full work"}</button>
          </div>
        ) : null}
      </div>
      ...
      {(trailer || preview) && introChoice === "main" ? (
        <div className={styles.introBar}>
          ...
          {trailer ? <button type="button" onClick={() => setIntroChoice("trailer")}>{ar ? "شاهد التريلر" : "Watch trailer"}</button> : null}
          {preview ? <button type="button" onClick={() => setIntroChoice("preview")}>{ar ? "شاهد المعاينة" : "Watch preview"}</button> : null}
          <button className={styles.primaryChoice} type="button" onClick={() => setIntroChoice("main")}>{ar ? "ابدأ العمل" : "Start work"}</button>
        </div>
      ) : null}
      ...
      <div className={styles.controls}>
        ...
        <div className={styles.controlRow}>
          ...
          <button type="button" onClick={() => setShowChapters((value) => !value)} aria-pressed={showChapters} title={ar ? "الفصول" : "Chapters"}><List size={17} /></button>
          <button type="button" onClick={() => setShowSettings((value) => !value)} aria-pressed={showSettings} title={ar ? "الإعدادات" : "Settings"}><Settings2 size={17} /></button>

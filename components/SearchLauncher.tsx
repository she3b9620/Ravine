...
const qualities: Option[] = [
  { value: "144p", ar: "144p", en: "144p" },
  { value: "240p", ar: "240p", en: "240p" },
  { value: "360p", ar: "360p", en: "360p" },
  { value: "480p", ar: "480p", en: "480p" },
  { value: "720p", ar: "720p", en: "720p" },
        ...
        >
          <div className={`ravine-search-dialog${advanced ? " is-advanced" : ""}${closing ? " is-closing" : ""}`} role="dialog" aria-modal="true" aria-labelledby="ravine-search-title" dir={ar ? "rtl" : "ltr"}>
            ...
            <form onSubmit={submit} className="ravine-search-form">
              ...
              <button type="button" className={`ravine-advanced-search-toggle${advanced ? " active" : ""}`} onClick={() => setAdvanced(!advanced)} aria-expanded={advanced}>
...
function SearchSelect({
  ...
  return (
    <div ref={rootRef} className={`ravine-search-select${open ? " is-open" : ""}${value ? " has-value" : ""}${disabled ? " is-disabled" : ""}`}>
      ...
      <button type="button" className="ravine-search-select-trigger" onClick={toggle} aria-expanded={open} disabled={disabled}>
      ...
      {open ? (
        <div className="ravine-search-select-menu" role="listbox">
          ...
          {options.map((item) => (
            <button type="button" key={item.value} className={value === item.value ? "is-selected" : ""} onClick={() => { onChange(item.value); setOpen(false); }}>

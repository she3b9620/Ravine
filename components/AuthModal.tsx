...
export default function AuthModal({ open, mode: initialMode = "signin", onClose }: Props) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (!firstName.trim() || !lastName.trim() || !username.trim()) {
          throw new Error(isArabic ? "أكمل الاسم الأول واسم العائلة واسم المستخدم." : "Complete your first name, last name, and username.");
        }
        ...
        if (data.session) {
          ...
          setSignupStep("email");
          setMessage(isArabic ? "أرسلنا رسالة تأكيد إلى بريدك الإلكتروني. بعد التأكيد سيظهر لك خيار تفعيل التحقق بخطوتين." : "We sent a confirmation email. After confirming your email, you can enable two-step verification.");
        }
      } else {
  ...
  function renderAuthForm() {
    if (signupStep === "email" && mode === "signup") {
      return (
        <div className="mt-6 rounded-3xl border p-5 text-center" style={{ borderColor: "rgba(241,233,220,.12)", background: "rgba(241,233,220,.035)" }}>
          <div className="mx-auto grid h-12 w-12 place-items-center rounded-full" style={{ background: "rgba(196,122,82,.14)", color: "#C47A52" }}><Check size={22} /></div>
          <h3 className="mt-4 text-lg font-black">{isArabic ? "تحقق من بريدك" : "Verify your email"}</h3>
          <p className="mt-2 text-sm leading-6" style={{ color: "rgba(241,233,220,.58)" }}>{message}</p>
          <button type="button" onClick={onClose} className="mt-5 w-full rounded-2xl px-4 py-3 text-sm font-black" style={{ background: "#C47A52", color: "#090909" }}>{isArabic ? "تم" : "Done"}</button>
        </div>
      );
    }
    ...
    if (signupStep === "mfa") {
      return (
        <div className="mt-6">
          {mfaQr && <div className="mx-auto mb-5 grid w-fit place-items-center rounded-3xl bg-white p-4"><img src={mfaQr.startsWith("data:") ? mfaQr : `data:image/svg+xml;utf8,${encodeURIComponent(mfaQr)}`} alt={isArabic ? "رمز QR لتطبيق المصادقة" : "Authenticator QR code"} className="h-48 w-48" /></div>}
          {!mfaQr && <div className="mx-auto mb-5 grid h-20 w-20 place-items-center rounded-3xl" style={{ background: "rgba(196,122,82,.12)", color: "#C47A52" }}><QrCode size={34} /></div>}
          <h3 className="text-center text-lg font-black">{isArabic ? "فعّل التحقق بخطوتين" : "Enable two-step verification"}</h3>
          <p className="mt-2 text-center text-sm leading-6" style={{ color: "rgba(241,233,220,.58)" }}>
            {message || (isArabic ? "امسح الرمز بتطبيق Google Authenticator ثم اكتب الرمز المكوّن من 6 أرقام." : "Scan the QR code with Google Authenticator, then enter the 6-digit code.")}
          </p>
      ...
      <>
        ...
        <form onSubmit={submit}>
          {mode === "signup" && <div className="grid grid-cols-2 gap-3">
            <label className="block text-sm font-semibold">{isArabic ? "الاسم الأول" : "First name"}<input required value={firstName} onChange={(event) => setFirstName(event.target.value)} className="mt-2 w-full rounded-2xl border px-4 py-3 outline-none" style={{ borderColor: "rgba(241,233,220,.12)", background: "rgba(241,233,220,.035)", color: "#F1E9DC" }} /></label>
            <label className="block text-sm font-semibold">{isArabic ? "اسم العائلة" : "Last name"}<input required value={lastName} onChange={(event) => setLastName(event.target.value)} className="mt-2 w-full rounded-2xl border px-4 py-3 outline-none" style={{ borderColor: "rgba(241,233,220,.12)", background: "rgba(241,233,220,.035)", color: "#F1E9DC" }} /></label>
            <label className="col-span-2 block text-sm font-semibold">{isArabic ? "اسم المستخدم" : "Username"}<input required minLength={3} value={username} onChange={(event) => setUsername(event.target.value)} placeholder="@username" className="mt-2 w-full rounded-2xl border px-4 py-3 outline-none" style={{ borderColor: "rgba(241,233,220,.12)", background: "rgba(241,233,220,.035)", color: "#F1E9DC" }} /></label>
          </div>}
        ...
        <button type="button" onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setSignupStep("form"); clearFeedback(); }} className="mt-5 w-full text-sm" style={{ color: "rgba(241,233,220,.58)" }}>
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] grid min-h-[100dvh] w-screen place-items-center overscroll-contain overflow-y-auto bg-black/70 p-4 backdrop-blur-md" style={{ position: "fixed", inset: 0, width: "100vw", height: "100dvh" }} role="dialog" aria-modal="true" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div className="relative my-auto max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-[32px] border p-6 shadow-2xl" style={{ background: "#111516", borderColor: "rgba(241,233,220,.12)", color: "#F1E9DC" }} onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between">
          <div><p className="text-[10px] font-bold uppercase tracking-[.28em]" style={{ color: "#C47A52" }}>RAVINE</p><h2 className="mt-2 text-2xl font-black">{signupStep === "mfa" ? (isArabic ? "حماية الحساب" : "Secure your account") : mode === "signin" ? (isArabic ? "مرحبًا بعودتك" : "Welcome back") : (isArabic ? "أنشئ حسابك" : "Create your account")}</h2></div>
          <button type="button" onClick={onClose} className="rounded-full border p-2" style={{ borderColor: "rgba(241,233,220,.12)" }} aria-label={isArabic ? "إغلاق" : "Close"}><X size={17} /></button>
        </div>
        {renderAuthForm()}
      </div>
    </div>,

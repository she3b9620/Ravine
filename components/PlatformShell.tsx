...
export default function PlatformShell({ children, active = "", title, eyebrow, description }: Props) {
  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      <a
        ...
        style={{
          background: active === item.key ? `${accent}18` : "transparent",
          color: active === item.key ? accent : muted,
        }}
      >
        <span
          className="flex h-9 w-9 items-center justify-center rounded-xl"
          style={{ background: active === item.key ? `${accent}22` : "transparent" }}
        >
          <Icon size={17} />
        </span>
  ...
  return (
    <div dir={isArabic ? "rtl" : "ltr"} className="min-h-screen" style={{ background: bg, color: text }}>
      <div
        className="fixed inset-x-0 top-0 z-[80] h-px"
        style={{ background: `linear-gradient(90deg, transparent, ${accent}, transparent)` }}
      />

      <header
        className="sticky top-0 z-50 border-b backdrop-blur-2xl"
        style={{ background: dark ? "rgba(9,9,9,.82)" : "rgba(241,233,220,.84)", borderColor: line }}
      >
        <div className="mx-auto flex h-[74px] max-w-[1600px] items-center gap-3 px-4 sm:px-6 lg:px-8">
          <button
            onClick={() => setOpen(true)}
            className="flex h-10 w-10 items-center justify-center rounded-2xl border lg:hidden"
              ...
              {mainItems.slice(0, 5).map((item) => {
                ...
                return (
                  <a
                    key={item.key}
                    href={href(item.path)}
                    className="flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold transition hover:-translate-y-0.5"
                    style={{
                      background: active === item.key ? `${accent}22` : "transparent",
                      color: active === item.key ? accent : muted,
                    }}
                  >
                    <Icon size={15} />
          ...
          <div className="ms-auto flex items-center gap-2">
            <a
              href={href("/search")}
              className="flex h-10 w-10 items-center justify-center rounded-full border sm:w-auto sm:px-3"
              style={{ borderColor: line, background: panel }}
            ...
            {userName ? (
              ...
              <a href={href("/auth")} className="rounded-full px-4 py-2 text-xs font-bold" style={{ background: accent, color: bg }}>
      ...
      <aside
        className="fixed bottom-0 start-0 top-[74px] z-40 hidden w-[250px] border-e p-4 lg:block"
        style={{ borderColor: line, background: dark ? "rgba(9,9,9,.72)" : "rgba(241,233,220,.72)" }}
      >
        <div className="flex h-full flex-col">
          <div className="space-y-1">{mainItems.map(renderItem)}</div>
          <div className="my-5 h-px" style={{ background: line }} />
          <div className="space-y-1">
            <a href={href("/library")} className="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm" style={{ color: muted }}>
              <span className="flex h-9 w-9 items-center justify-center rounded-xl"><Library size={17} /></span>
              {isArabic ? "مكتبتي" : "Library"}
            </a>
            <a href={href("/account/security")} className="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm" style={{ color: muted }}>
              <span className="flex h-9 w-9 items-center justify-center rounded-xl"><Settings2 size={17} /></span>
      ...
      {open && (
        <div className="fixed inset-0 z-[90] bg-black/60 backdrop-blur-sm lg:hidden" onClick={() => setOpen(false)}>
          <aside
            className="h-full w-[min(86vw,320px)] border-e p-5"
            style={{ background: bg, borderColor: line }}
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-center justify-between">
              <img src="/RAVINE.png" alt="RAVINE" className="h-11 w-auto" />
              <button
                onClick={() => setOpen(false)}
                className="flex h-10 w-10 items-center justify-center rounded-full border"
            ...
            <div className="mt-8 space-y-1">{mainItems.map(renderItem)}</div>
          </aside>
        </div>
      )}
      ...
      <main className="lg:ps-[250px]">
        {(title || eyebrow) && (
          <div className="mx-auto max-w-[1440px] px-5 pb-2 pt-10 md:px-8 lg:px-10">

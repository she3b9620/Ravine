...
export type HomeWork = { id: number; title: string | null; description: string | null; thumbnail_url: string | null; video_url: string | null; duration: number | null; views: number | null; likes: number | null; content_type: string | null; quality: string | null; creator_id: number | null; creators: WorkCreator[] | WorkCreator | null };
function getCreator(creator: HomeWork["creators"]) { return Array.isArray(creator) ? creator[0] ?? null : creator; }
function formatDuration(seconds: number | null) { if (!seconds || seconds < 1) return "—"; const total = Math.round(seconds); const h = Math.floor(total / 3600); const m = Math.floor((total % 3600) / 60); const s = total % 60; return h > 0 ? `${h}:${String(m).padStart(2,"0")}:${String(s).padStart(2,"0")}` : `${m}:${String(s).padStart(2,"0")}`; }
function contentTypeLabel(value: string | null, locale: Locale) { if (locale !== "ar") return value || "WORK"; const labels: Record<string,string> = { short:"قصير", video:"فيديو", film:"فيلم", documentary:"وثائقي", podcast:"برنامج صوتي", live:"جلسة مباشرة", photo:"صورة", work:"عمل" }; return labels[value?.toLowerCase() || "work"] || value || "عمل"; }
function qualityLabel(value: string | null, locale: Locale) { if (locale !== "ar") return value || ""; const labels: Record<string,string> = { hd:"HD", fullhd:"1080p", uhd:"UHD", "4k":"4K", "8k":"8K" }; return value ? labels[value.toLowerCase()] || value : ""; }
function isYouTubeUrl(value: string | null) { if (!value) return false; try { const host = new URL(value, window.location.origin).hostname.toLowerCase().replace(/^www\./,""); return host === "youtube.com" || host === "youtu.be" || host.endsWith(".youtube.com"); } catch { return false; } }

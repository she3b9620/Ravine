...
export default function PrimaryNav({ locale }: { locale: Locale }) {
  ...
  return (
    <nav className="ravine-sidebar-nav" aria-label={locale === "ar" ? "التنقل الرئيسي" : "Primary navigation"}>
      <div className="ravine-sidebar-nav-label">{locale === "ar" ? "التنقل" : "Navigate"}</div>
      <div className="ravine-sidebar-nav-list">
        {navigation.map(([slug, en, ar, Icon]) => {
          ...
          return (
            ...
            >
              <span className="ravine-sidebar-link-icon">

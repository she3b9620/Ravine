...
export default function AccountSettings({profile,locale}:Props){
  ...
  return <div className={styles.settingsShell}>
    ...
    <section className={styles.sectionBlock}><h3 className={styles.sectionLabel}>{ar?"الصور":"Media"}</h3><div className={styles.mediaGrid}><RavineMediaPicker aspect="square" locale={locale} existingUrl={profile?.avatar_url} value={avatar} resetKey={mediaResetKey} onChange={setAvatar}/><RavineMediaPicker aspect="cover" locale={locale} existingUrl={profile?.cover_url} value={cover} resetKey={mediaResetKey} onChange={setCover}/></div></section>

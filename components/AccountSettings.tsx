...
export default function AccountSettings({profile,locale}:Props){
  ...
  const [savedCoverUrl,setSavedCoverUrl]=useState(profile?.cover_url||null);
  ...
  function cancelChanges(){if(busy)return;setDisplayName(profile?.display_name||"");setUsername(profile?.username||"");setBio(profile?.bio||"");setCountry(profile?.country||"");setLanguage(profile?.language==="en"?"en":"ar");setWebsite(profile?.website_url||"");setTrailer(profile?.trailer_url||"");setTimezone(profile?.timezone||"");setAvatar(null);setCover(null);setSavedAvatarUrl(profile?.avatar_url||null);setSavedCoverUrl(profile?.cover_url||null);setMediaResetKey(v=>v+1);setMessage("");setError("")}
  async function save(){setBusy(true);setError("");setMessage("");const s=createClient(),{data:auth}=await s.auth.getUser();if(!auth.user){setError(ar?"يجب تسجيل الدخول أولًا.":"You must be signed in first.");setBusy(false);return}const uploaded:Array<{bucket:"avatars"|"covers";path:string}>=[];try{let avatarUrl=savedAvatarUrl,coverUrl=savedCoverUrl;if(avatar){const u=await uploadAsset(auth.user.id,"avatars",avatar);avatarUrl=u.url;uploaded.push({bucket:"avatars",path:u.path})}if(cover){const u=await uploadAsset(auth.user.id,"covers",cover);coverUrl=u.url;uploaded.push({bucket:"covers",path:u.path})}const patch={display_name:displayName.trim()||null,username:username.trim().replace(/^@+/ ,"")||null,bio:bio.trim()||null,country:country||null,language:language as "ar"|"en",website_url:website.trim()||null,avatar_url:avatarUrl,cover_url:coverUrl,timezone:timezone||browserTimeZone(),trailer_url:profile?.is_creator?trailer.trim()||null:null};const{error:updateError}=await s.from("profiles").update(patch).eq("id",auth.user.id);if(updateError)throw updateError;setSavedAvatarUrl(avatarUrl);setSavedCoverUrl(coverUrl);setAvatar(null);setCover(null);setMessage(ar?"تم حفظ التعديلات بنجاح.":"Changes saved successfully.")}catch(e){for(const item of uploaded)await s.storage.from(item.bucket).remove([item.path]);setError(e instanceof Error?e.message:String(e))}finally{setBusy(false)}}
  return <div className={styles.settingsShell}>
    ...
    <section className={styles.sectionBlock}><h3 className={styles.sectionLabel}>{ar?"الصور":"Media"}</h3><div className={styles.mediaGrid}><RavineMediaPicker aspect="square" locale={locale} existingUrl={savedAvatarUrl} value={avatar} resetKey={mediaResetKey} onChange={setAvatar}/><RavineMediaPicker aspect="cover" locale={locale} existingUrl={savedCoverUrl} value={cover} resetKey={mediaResetKey} onChange={setCover}/></div></section>
    ...
    <div className={styles.actions}><a className={styles.actionLink} href={publicProfileHref(locale,savedUsername(username))}><span>{ar?"عرض صفحتي":"View my page"}</span><ArrowUpRight size={14}/></a><div className={styles.actionsRight}><button className={`${styles.button} ${styles.secondaryButton}`} type="button" onClick={cancelChanges} disabled={busy}><RotateCcw size={14}/><span>{ar?"إلغاء التعديلات":"Discard changes"}</span></button><button className={`${styles.button} ${styles.buttonPrimary}`} type="button" onClick={save} disabled={busy}>{busy?(ar?"جارٍ الحفظ…":"Saving…"):(ar?"حفظ التغييرات":"Save changes")}</button></div></div>
  </div>
}

function savedUsername(value:string){return value.trim().replace(/^@+/ ,"")}
function publicProfileHref(locale:string,username:string){return username?`/${locale}/u/${encodeURIComponent(username)}`:`/${locale}/account`}

...
export default function NotificationBell({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, count: unread } = await supabase.from("notifications").select("id,title,body,is_read,created_at,video_id", { count: "exact" }).eq("user_id", auth.user.id).order("created_at", { ascending: false }).limit(6);
      if (!mounted) return;
      const rows = (data ?? []) as Notification[];
      setItems(rows); setCount(unread || 0);
      channel = supabase.channel(`ravine-notifications:${auth.user.id}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications", filter: `user_id=eq.${auth.user.id}` }, (payload) => {
        const notification = payload.new as Notification;
        setItems((current) => [notification, ...current.filter((item) => item.id !== notification.id)].slice(0, 6)); setCount((current) => current + 1);
      }).subscribe();
    }
    void load();
  ...
  useEffect(() => { function onPointerDown(event: PointerEvent) { if (!rootRef.current?.contains(event.target as Node)) close(); } document.addEventListener("pointerdown", onPointerDown); return () => document.removeEventListener("pointerdown", onPointerDown); }, [open, closing]);
  ...
  return (
    <div ref={rootRef} className="ravine-notification-wrap">
      <button type="button" className="ravine-header-icon ravine-notification-bell" aria-label={ar ? "الإشعارات" : "Notifications"} title={ar ? "الإشعارات" : "Notifications"} aria-expanded={open && !closing} onClick={toggle}>
        <Bell size={18} strokeWidth={1.8} />{count > 0 ? <span className="ravine-header-icon-badge">{count > 99 ? "99+" : count}</span> : null}
      </button>
      {open || closing ? (
        <div className={`ravine-notification-popover${closing ? " is-closing" : ""}`} dir={ar ? "rtl" : "ltr"} role="dialog" aria-label={ar ? "معاينة الإشعارات" : "Notification preview"}>
          ...
          <div className="ravine-notification-popover-list">
            {items.length ? items.map((item) => {
              ...
              if (item.video_id) return <Link key={item.id} href={`/${locale}/watch/${item.video_id}`} onClick={() => { void markRead(item.id); close(); }}>{content}</Link>;
              return <button key={item.id} type="button" className="ravine-notification-preview-button" onClick={() => void markRead(item.id)}>{content}</button>;
            }) : <div className="ravine-notification-empty">{ar ? "لا توجد إشعارات جديدة." : "No notifications yet."}</div>}
          </div>
          <Link className="ravine-notification-open" href={`/${locale}/notifications`} onClick={close}><span>{ar ? "فتح صفحة الإشعارات" : "Open notifications"}</span><ExternalLink size={14} /></Link>
        </div>
      ) : null}
    </div>
  );
